...
import {
  ...
  Pencil, Trash2, MapPin, Calendar, ChevronLeft, ChevronRight, Package,
} from "lucide-react";
                    ...
                    <tr key={batch.id} className="hover:bg-slate-50/80 transition-colors" style={{ animation: `fadeInUp 0.3s ease-out ${index * 0.05}s both` }}>
                      ...
                      <td className="px-6 py-4 whitespace-nowrap text-right">
                        <div className="flex items-center justify-end gap-2">
                          <Button variant="ghost" size="sm" onClick={() => handleEditBatch(batch)} className="hover:bg-blue-50 hover:text-blue-600">
                            <Pencil className="w-4 h-4" />
                          </Button>
                          <Button variant="ghost" size="sm" onClick={() => handleDeleteClick(batch)} className="hover:bg-red-50 hover:text-red-600">
            ...
            return (
              ...
              >
                ...
                <div className="flex gap-2 mt-4 pt-4 border-t border-slate-100">
                  <Button variant="ghost" size="sm" onClick={() => handleEditBatch(batch)} className="flex-1 hover:bg-blue-50 hover:text-blue-600">
                    <Pencil className="w-4 h-4 mr-1" />
                    Edit
                  </Button>
                  <Button variant="ghost" size="sm" onClick={() => handleDeleteClick(batch)} className="flex-1 hover:bg-red-50 hover:text-red-600">
          ...
          {paginatedBatches.length > 0 && (
            <div className="md:col-span-2 flex items-center justify-between bg-white rounded-2xl border border-slate-200 px-4 py-3">
              <p className="text-xs text-slate-600">
                {startIndex + 1}-{Math.min(endIndex, filteredBatches.length)} of {filteredBatches.length}
              </p>
              <div className="flex items-center gap-2">
                <Button variant="outline" size="sm" onClick={() => goToPage(currentPage - 1)} disabled={currentPage === 1} className="border-slate-200">
                  <ChevronLeft className="w-4 h-4" />
        ...
        {filteredBatches.length === 0 && (
          <div className="text-center py-16 bg-white rounded-2xl border border-slate-200">
            ...
            <p className="text-slate-500 mb-4">
              {searchQuery || statusFilter !== "All" || medicineFilter !== "All"
                ? "Try adjusting your filters"
                : "Get started by adding your first batch"}
            </p>
            {!searchQuery && statusFilter === "All" && medicineFilter === "All" && (
              <Button onClick={handleAddBatch} className="bg-gradient-to-r from-blue-600 to-cyan-600 hover:from-blue-700 hover:to-cyan-700">
                <Plus className="w-4 h-4 mr-2" />
                Add Batch
              </Button>
            )}
