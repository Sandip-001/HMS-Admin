// app/(main)/emergency/dashboard/_components/emergency-bed-occupancy-grid.tsx
"use client";
...
import type { BedDetail } from "@/types/emergency/emergency-analytics-types";
...
const TRIAGE_STYLES: Record<NonNullable<BedDetail["triageLevel"]>, string> = {
  Critical: "border-red-400 bg-red-100 text-red-800",
  Urgent: "border-amber-400 bg-amber-100 text-amber-800",
  Standard: "border-emerald-400 bg-emerald-100 text-emerald-800",
};

export function EmergencyBedOccupancyGrid({
  beds,
  onOpenDrawer,
}: {
  beds: BedDetail[];
      ...
      <CardHeader className="pb-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <CardTitle className="flex items-center gap-2 text-base font-bold text-slate-800">
            <BedDouble className="h-5 w-5 text-rose-500" /> Live Emergency Bed Map
          </CardTitle>
            ...
            {(["All", "Occupied", "Available", "Maintenance"] as const).map((status) => (
              <button
                ...
                className={`rounded-md px-2.5 py-1.5 text-xs font-semibold transition-all ${
                  filter === status ? "bg-white text-rose-700 shadow-sm" : "text-slate-500"
                }`}
              >
                {status}
              </button>
                    ...
                    >
                      ...
                      {bed.status === "Occupied" ? (
                        <div>
                          ...
                          {bed.triageLevel ? (
                            <Badge variant="outline" className={`${TRIAGE_STYLES[bed.triageLevel]} mt-1 text-[9px]`}>
                              {bed.triageLevel}
                            </Badge>
                          ) : null}
                        </div>
