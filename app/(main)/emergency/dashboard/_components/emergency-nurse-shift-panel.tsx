// app/(main)/emergency/dashboard/_components/emergency-nurse-shift-panel.tsx
"use client";
...
import { exportNurseAssignmentsToExcel } from "@/lib/emergency/emergency-excel-export";
...
export function EmergencyNurseShiftPanel({
  assignments,
  onViewAll,
}: {
              ...
              {(["All", "Morning", "Evening", "Night"] as const).map((shift) => (
                <button
                  ...
                  className={`rounded-md px-2.5 py-1.5 text-xs font-semibold transition-all ${
                    shiftFilter === shift ? "bg-white text-rose-700 shadow-sm" : "text-slate-500"
                  }`}
                >
                  {shift}
                </button>
