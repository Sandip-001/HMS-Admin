// app/(main)/emergency/dashboard/_components/emergency-doctor-workload-table.tsx
"use client";
...
import type { DoctorEmergencyStatus, DoctorEmergencyWorkload } from "@/types/emergency/emergency-analytics-types";
import { exportDoctorWorkloadToExcel } from "@/lib/emergency/emergency-excel-export";

const STATUS_STYLES: Record<DoctorEmergencyStatus, string> = {
...
export function EmergencyDoctorWorkloadTable({
  doctors,
  onViewAll,
}: {
  doctors: DoctorEmergencyWorkload[];
  onViewAll: () => void;
}) {
      ...
      <CardHeader className="pb-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <CardTitle className="flex items-center gap-2 text-base font-bold text-slate-800">
            <Trophy className="h-5 w-5 text-amber-500" /> Doctor-wise Emergency Patient Load
          </CardTitle>
              ...
              {(["patients", "revenue", "critical"] as const).map((key) => (
                <button
                  ...
                  className={`rounded-md px-2.5 py-1.5 text-xs font-semibold capitalize transition-all ${
                    sortBy === key ? "bg-white text-rose-700 shadow-sm" : "text-slate-500"
                  }`}
                >
                  {key}
                </button>
