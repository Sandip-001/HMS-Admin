// app/(dashboard)/super-admin/opd-analytics/_components/opd-medicine-lab-tables.tsx
"use client";
...
import type { LabTestData, MedicineSalesData, TrendDirection } from "@/types/opd/opd-analytics-types";
import { exportLabTestsToExcel, exportMedicineSalesToExcel } from "@/lib/opd/opd-excel-export";
...
export function OPDMedicineSalesTable({ medicines }: { medicines: MedicineSalesData[] }) {
      ...
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <CardTitle className="text-base font-bold text-slate-800 flex items-center gap-2">
            <Pill className="h-5 w-5 text-violet-500" /> Pharmacy — Medicine Sales
          </CardTitle>
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center justify-between mb-1">
                    ...
                    <div className="flex items-center gap-2 shrink-0 ml-2">
                      <Badge variant="outline" className={`${trendBadgeClass(med.trend)} text-xs gap-1`}>
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-xs text-slate-500 shrink-0 w-24 text-right">{med.unitsSold} units · ₹{med.revenue.toLocaleString()}</span>
                  </div>
...
export function OPDLabTestsTable({ tests }: { tests: LabTestData[] }) {
  ...
  const displayed = [...filtered].sort((a, b) => b.totalOrdered - a.totalOrdered).slice(0, 8);
  ...
  return (
    <Card className="border-slate-200 shadow-lg">
      <CardHeader className="pb-3">
        <div className="flex items-center justify-between flex-wrap gap-3">
          <CardTitle className="text-base font-bold text-slate-800 flex items-center gap-2">
            <FlaskConical className="h-5 w-5 text-cyan-500" /> Lab — Pathology &amp; Radiology
          </CardTitle>
          <div className="flex items-center gap-2">
            <div className="flex rounded-lg border border-slate-200 bg-slate-50 p-0.5">
          ...
          <table className="w-full">
            <thead>
              <tr className="border-b border-slate-200">
                ...
                <th className="text-center py-2.5 px-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">TAT</th>
            ...
            <tbody>
              {displayed.map((test) => (
                <tr key={test.id} className="border-b border-slate-100 hover:bg-slate-50 transition-colors">
                  ...
                  <td className="py-3 px-3 text-center text-xs text-slate-500">{test.avgTurnaroundTime}</td>
