// app/(main)/icu/dashboard/_components/icu-medicine-lab-tables.tsx
"use client";
...
import { exportLabTestsToExcel, exportMedicineSalesToExcel } from "@/lib/icu/icu-excel-export";
...
export function ICUMedicineSalesTable({
  medicines,
  onViewAll,
}: {
  medicines: IcuMedicineSalesData[];
  onViewAll: () => void;
}) {
      ...
      <CardHeader className="pb-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <CardTitle className="flex items-center gap-2 text-base font-bold text-slate-800">
            <Pill className="h-5 w-5 text-violet-500" /> Pharmacy — ICU Medicine Sales
          </CardTitle>
...
export function ICULabTestsTable({
  tests,
  onViewAll,
}: {
  tests: IcuLabTestData[];
  onViewAll: () => void;
}) {
  ...
  return (
    <Card className="border-slate-200 shadow-lg">
      <CardHeader className="pb-3">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <CardTitle className="flex items-center gap-2 text-base font-bold text-slate-800">
            <FlaskConical className="h-5 w-5 text-cyan-500" /> Lab — ICU Pathology &amp; Radiology
          </CardTitle>
              ...
              {(["All", "Pathology", "Radiology"] as const).map((type) => (
                <button
                  ...
                  className={`rounded-md px-2.5 py-1.5 text-xs font-semibold transition-all ${
                    filterType === type ? "bg-white text-red-700 shadow-sm" : "text-slate-500"
                  }`}
                >
                  {type}
                </button>
